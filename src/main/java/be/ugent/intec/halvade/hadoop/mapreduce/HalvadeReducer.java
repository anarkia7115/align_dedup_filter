...
public class HalvadeReducer extends Reducer<ChromosomeRegion, SAMRecordWritable, ChromosomeRegion, VariantContextWritable> {
    ...
    protected int count;
    protected ArrayList<String> bamFiles;
    ...
    protected void cleanup(Context context) throws IOException, InterruptedException {
        ...
        String output = null;
        if(bamFiles.size() > 1) { // should not happen -> multiple keys per reducer
            output = tmp + context.getTaskAttemptID().toString() + ".bam";
            Logger.DEBUG("Multi bam files per reducer task, error");
            context.getCounter(HalvadeCounters.TOOLS_GATK).increment(1);
        } else if (bamFiles.size() == 1) {
            output = bamFiles.get(0);
        }
        if(output != null) { 
            try {
                HalvadeFileUtils.uploadFileToHDFS(context, FileSystem.get(new URI(outputdir), context.getConfiguration()),
                        output, outputdir + context.getTaskAttemptID().toString() + ".bam");
                // remove result bam files
                HalvadeFileUtils.removeLocalFile(keep, output, context, HalvadeCounters.FOUT_GATK_TMP);
            } catch (URISyntaxException ex) {
                Logger.EXCEPTION(ex);
                throw new InterruptedException();
            }
        } else {
            Logger.DEBUG("empty bam file, not uploaded to bam to avoid error when further analysis.");
        }

    }
    
    protected boolean checkBamIsNotEmpty(String samFile) throws FileNotFoundException, IOException {
        int recordCount = 0;
        try (BufferedReader br = new BufferedReader(new FileReader(samFile))) {
