...
public class SAMStreamHandler extends Thread {
	...
	/*
	 * reads input from an outputstream (stdout from a process) writes every
	 * input from the stream to the output of hadoop which will be sorted and
	 * processed further
	 */
	InputStream is;
		...
		try {
			while (mCurrentLine != null) {
				try {
					...
					// paired or unpaired ?? need to know to check for
					// boundaries

					if (isPaired)
						count += instance.writePairedSAMRecordToContext(samrecord, useCompact);
					else
						count += instance.writeSAMRecordToContext(samrecord, useCompact);
				} catch (SAMFormatException se) {
					Logger.DEBUG("current line: " + mCurrentLine);
					Logger.EXCEPTION(se);
				}
