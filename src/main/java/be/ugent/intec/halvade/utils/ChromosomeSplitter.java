...
public class ChromosomeSplitter {
	...
	private void calculateRegionsPerChromosome(String bedFile) throws IOException { // ,
																					// boolean
																					// coverAll
		regions = new HashMap<String, ArrayList<BedRegion>>();
		...
		try {
			...
			while (line != null) {
				...
				if (!bedRegion[0].equalsIgnoreCase(currentContig)) {
					...
					if (dict.getSequence(currentContig) != null) {
						if (!regions.containsKey(currentContig))
							regions.put(currentContig, new ArrayList<BedRegion>());
						regions.get(currentContig).add(
								new BedRegion(currentStart, dict.getSequence(currentContig).getSequenceLength(), i));
						i++;
					}
					currentContig = bedRegion[0];
					currentStart = 0;
					...
					// new genome partition method, single arm one reducer!
					if (dict.getSequence(currentContig) != null) {
						if (!regions.containsKey(currentContig))
							regions.put(currentContig, new ArrayList<BedRegion>());
					...
					currentContig = bedRegion[0];
					currentStart = currentEnd - MIN_GENE_SEPARATION;
					currentEnd = end;
				}
				line = br.readLine();
			}
			if (dict.getSequence(currentContig) != null) {
				if (!regions.containsKey(currentContig))
					regions.put(currentContig, new ArrayList<BedRegion>());
				regions.get(currentContig)
						.add(new BedRegion(currentStart, dict.getSequence(currentContig).getSequenceLength(), i));
			}
			for (Entry<String, ArrayList<BedRegion>> region : regions.entrySet()) {
				...
				for (BedRegion breg : region.getValue()) {
					regionCount++;
					Logger.DEBUG("region: " + breg.key + ", " + contig + " (" + breg.start + " _ " + breg.end + " -> "
							+ (breg.end - breg.start) + ")", 2);
				}
			}
		} finally {
			br.close();
